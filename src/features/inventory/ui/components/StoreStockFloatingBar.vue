...
<template>
  <div v-if="(!isReadOnly && selectedIds.size > 0) || pasteNotification">
      ...
      >
        <div class="flex items-center gap-2.5">
          ...
          <span class="text-xs font-medium text-indigo-100">
            Выбрано позиций: <strong class="font-bold text-white">{{ selectedIds.size }}</strong> из {{ store.totalStockCount }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            title="Оставить по 1 записи на каждый товар и удалить лишние повторки"
          >
            <span>🗑️</span>
            <span>Оставить по 1 шт (удалить повторы)</span>
          </button>
    ...
    >
      <div
        ...
        class="flex items-center gap-2 rounded-lg bg-emerald-500/90 px-3 py-1.5 text-xs font-semibold text-white shadow-xl backdrop-blur-xs"
      >
        <span>📋</span>
