...
<template>
  <tr v-if="!isReadOnly" class="border-b border-indigo-500/30 bg-indigo-950/20 select-none">
    ...
    <td class="w-14 text-center text-indigo-400 font-bold text-xs">＋</td>

    <!-- SKU input -->
    <td class="px-2.5 py-1.5 w-36">
      <input
        v-model="newSku"
        type="text"
        ...
        class="w-full rounded bg-gray-950 px-2 py-1 text-xs font-mono font-bold text-indigo-300 ring-1 ring-indigo-500/50 focus:ring-indigo-400 placeholder:text-gray-600 focus:outline-none shadow-sm"
        title="Введите ЛК или вставьте столбец артикулов из Excel (Ctrl+V)"
      />
    </td>

    <!-- Name auto preview -->
    <td class="px-3 py-1.5 min-w-[200px] text-xs text-gray-400 truncate">
      <span v-if="newSku.trim() && store.catalogSkuMap.get(newSku.replace(/\D/g, '').slice(0, 7))" class="text-indigo-300 font-medium">
        {{ store.catalogSkuMap.get(newSku.replace(/\D/g, '').slice(0, 7)) }}
      </span>
      <span v-else-if="newSku.trim() && store.isNotFoundInCatalog(newSku.replace(/\D/g, '').slice(0, 7))" class="text-amber-400 font-medium">
        Н/Д (не в каталоге)
      </span>
      <span v-else class="text-gray-600 italic">Автоподстановка из каталога</span>
    </td>

    <!-- Mentions count preview -->
    <td class="px-2 py-1.5 text-center text-xs font-mono w-28">
    ...
    <!-- Fact Qty preview -->
    <td class="px-2 py-1.5 text-right text-xs font-mono w-32">
      <span v-if="newSku.trim()" class="text-indigo-300 font-semibold">
        {{ store.getFactSkuQuantity(newSku.replace(/\D/g, '').slice(0, 7)) }} шт.
      </span>
      <span v-else class="text-gray-600">—</span>
    </td>

    <!-- Multiplicity preview -->
    <td class="px-2 py-1.5 text-center text-xs font-mono w-24">
      <span v-if="newSku.trim()" class="text-indigo-300 font-semibold">
    ...
    <td class="px-2 py-1.5 text-right text-xs font-mono w-32">
      <span v-if="newSku.trim() && store.getStockItemQuantity(newSku.replace(/\D/g, '').slice(0, 7)) !== null" class="text-indigo-300 font-semibold">
        {{ store.getStockItemQuantity(newSku.replace(/\D/g, '').slice(0, 7)) }} шт.
      </span>
      <span v-else class="text-gray-600">—</span>
    </td>
    ...
    <!-- 299 preview -->
    <td class="px-2 py-1.5 text-center text-xs w-20">
      <span
        v-if="newSku.trim() && store.isAccount299Item(newSku.replace(/\D/g, '').slice(0, 7))"
        class="rounded bg-purple-500/20 text-purple-300 px-1.5 py-0.5 text-[10px] font-bold ring-1 ring-purple-500/40"
      >
        299
      </span>
      <span v-else class="text-gray-600">—</span>
    ...
    <!-- Date preview -->
    <td class="px-2 py-1.5 text-center text-xs font-mono w-36 text-gray-600">—</td>
    ...
    <td class="px-2 py-1.5 text-center w-14">
      <button
        ...
        class="w-full rounded bg-indigo-600 px-2 py-1 text-xs font-medium text-white hover:bg-indigo-500 disabled:opacity-30 transition-colors cursor-pointer shadow-sm"
